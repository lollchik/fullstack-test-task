...
class StoredFile(Base):
    ...
    alerts: Mapped[List["Alert"]] = relationship(back_populates="file")


class Alert(Base):
    __tablename__ = "alert"

    id: Mapped[int] = mapped_column(UUID, primary_key=True, default=uuid_utils.uuid7)
    file_id: Mapped[str] = mapped_column(UUID, ForeignKey("file.id"), nullable=False)
    ...
    file: Mapped[Optional["StoredFile"]] = relationship(back_populates="alerts")
